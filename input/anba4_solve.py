...
def get_material_db(material_map, unit_factor=1):
    ...
    for mat_name, mesh_id in mm.items():
        ...
        if matdb_id not in mat_db:
            ...
            msg = f"Material ID {matdb_id} not found in materials.yaml"
            raise KeyError(msg)
        ...
        if "Ex" in matdb_entry and "Ey" in matdb_entry and "Ez" in matdb_entry:
            ...
            if not all(k in matdb_entry for k in required_keys):
                msg = f"Missing orthotropic properties for material {matdb_id}: {required_keys}"
            ...
            if "E" not in matdb_entry and "Ex" not in matdb_entry:
                msg = f"Missing 'E' or 'Ex' for isotropic material {matdb_id}. Entry: {matdb_entry}"
                raise ValueError(msg)
            if "nu" not in matdb_entry:
                msg = f"Missing 'nu' for isotropic material {matdb_id}. Entry: {matdb_entry}"
                raise ValueError(msg)
...
def solve_anba4(mesh_filename, material_db_filename):
    ...
    for mesh_id in material_unique_indices:
        ...
        if mesh_id_str not in mm_inv:
            ...
            msg = f"Mesh material ID {mesh_id_str} not found in material map"
            raise KeyError(msg)
...
def run_solver(mesh, material_db):
    ...
    try:
        ...
        logger.exception(f"Failed to process {mesh}: {e!s}\n{traceback.format_exc()}")
